...
def upload():
    ...
    try:
        ...
        serverLog.info(f'task name: {fileName}{format} added')
...
def on_join(data):
    print("recived socket request")
    print("recived socket request")
    global tasks,serverLog
    serverLog.info('recived socket request')    
    fileName = data['fileName']
    task=tasks[fileName]
    join_room(fileName)
    print({"obj":task.prompt, "nextStep": task.nextStep ,'status': task.processing })
    if 'current' in task.prompt:
        serverLog.info({"obj":task.prompt, "nextStep": task.nextStep ,'status': task.processing })    
